...
void SetOfInts3::probar(){

	SetOfInts3 primero, segundo;
	...
	if (primero < segundo )
		cout << "PRIMERO ES UN SUBCONJUNTO DE SEGUNDO";
	else
		cout << "NOOOO";
}
...
bool  SetOfInts3:: operator==(const SetOfInts3& set){
	...
	if (size != set.size)
		...
		for (int i = 0; i < set.size && iguales; i++){
...
bool SetOfInts3:: operator<(const SetOfInts3& set){
	return (this->esSubconjunto(set));
}

bool SetOfInts3::esSubconjunto(const SetOfInts3& set){
	bool sub = true;
	for (int i = 0; i < size && sub; i++){
		sub = contains(set.elems[i]);
	}
