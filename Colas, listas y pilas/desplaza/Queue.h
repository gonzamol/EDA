...
public:
	...
	// LINEAL RESPECTO AL NUMERO DE ELEMENTOS DE LA COLA
	void desplaza(unsigned int pos, int dist){
		Nodo *act = _prim;
		Nodo *ant = _prim;
		...
		if(dist > 0){ // si la distancia es menor que 0, no nos molestamos ni en buscar una solucion
			// buscar el elemento de la posicion 'pos' que es el que hay que mover y el 'target' que es donde moverlo
			for(int i = 1; i <= _numElems && !enc; i++){
				if(pos - dist == i){ // pos - dist representa la posicion objetivo
					// la posicion objetivo siempre será un numero arriba de posicion donde debe quedar para guardar su siguiente y poder colocar los nodos
					beforeTarget = ant;
					target = act;
				}
		...
		//sale con el elemento en 'act' y donde ponerlo en 'target'
		if(enc){ // si lo ha encontrado
			if(dist >= _numElems || pos - dist <= 1 ){ // en caso de que haya que colocarlo el primero de la lista
				ant->_sig = act->_sig;
				act->_sig = _prim;
				_prim = act;				
			}
			else{
				// el siguiente del anterior apunta al siguiente del actual ESTO DEBE IR EN ESTA PRIMERA LINEA, ANTES DE MODIFICAR EL act->_sig
				ant->_sig = act->_sig; 
				act->_sig = target; // el siguiente del actual apunte al target
				beforeTarget->_sig = act; // el siguiente del anterior del target apunte al actual
			}
		}
