...
public:
	...
	void removeAll(const T &elem) {
		...
		//encontrar los elementos iguales al elem
		for (unsigned int i = 0; i < _numElems; i++){
			...
			if (nodo->_elem == elem){
				if (_prim == _ult){
					...
					if (nodo->_ant == NULL){
						nodo->_sig->_ant = nodo->_ant;
						_prim = nodo->_sig;
					}

					if (nodo->_sig == NULL){
						nodo->_ant->_sig = nodo->_sig;
						_ult = nodo->_ant;
					}

					if (nodo->_ant != NULL && nodo->_sig != NULL){
						nodo->_ant->_sig = nodo->_sig; // marca el siguiente del nodo anterior
						nodo->_sig->_ant = nodo->_ant; // marca el anterior del nodo siguiente
					}
				}
