...
public:
	...
	void duplica(){
		...
		while(act != NULL){
			nuevo = (Nodo*)malloc(sizeof(Nodo)); // creo el nodo
			nuevo->_elem = act->_elem; // le doy el mismo valor que el valor del actual
			nuevo->_sig = act->_sig; // su siguiente elemento será el siguiente del actual
			act->_sig = nuevo;	// el actual le apunta
			act = nuevo->_sig; // el nuevo actual es el siguiente del nuevo
			_numElems++; // aumentamos en 1 la cantida de elementos
		}
	}
