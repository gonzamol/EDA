...
protected:
	...
	void numHojasAux(List<T> l, Nodo* &raiz, int &num){
		if(raiz != NULL){
			if(raiz->_iz == NULL && raiz->_dr == NULL)
				num = num + 1;
			else{
				numHojasAux(l, raiz->_iz, num);
				numHojasAux(l, raiz->_dr, num);
			}
		}
	}

	void alturaAux(Nodo* &raiz, int &altura){
		if(raiz == NULL)
			altura = 0;
		else{
			int i, d;
			alturaAux(raiz->_iz, i);
			alturaAux(raiz->_dr, d);
	...
	void numNodosAux(Nodo* &raiz, int &num){
		if(raiz == NULL)
			num = 0;
		else{
			int i,d;
			numNodosAux(raiz->_iz, i);
			numNodosAux(raiz->_dr, d);
