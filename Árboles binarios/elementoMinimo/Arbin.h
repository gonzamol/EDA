...
protected:
	...
	//Auxiliar para el elemento menor
	void minElemAux(Nodo* &nodo, T &min){
		if(nodo->_iz == NULL && nodo->_dr == NULL) // si la raiz tiene null a ambos lados, ella misma sera el elemento minimo
			min = nodo->_elem;
		else{
			T minIz, minDr;
			if(nodo->_iz != NULL){
				minElemAux(nodo->_iz, minIz);
...
private:
	...
	static bool comparaMin(Nodo *r1, Nodo *r2) {
		if (r1 == r2)
			return true;
		else if ((r1 == NULL) || (r2 == NULL))
			// En el if anterior nos aseguramos de
